    ...
    <div class="panel-heading">
        ...
        <?php elseif ($model->type === History::TYPE_USER_ACTION): ?>
            ...
            <?php elseif ($model->item->type === Item::TYPE_RGB): ?></b>
                ...
                установил значение <b><?= $model->value ?></b> на RGB <b><?= $model->item->name ?></b> в комнате <b><?= $model->item->widget->room->name ?></b>
        ...
        <?php elseif ($model->type === History::TYPE_ITEM_VALUE): ?>
            ...
            <?php if ($model->item->type === Item::TYPE_SWITCH): ?>
                ...
                в комнате <b><?= $model->item->widget->room->name ?></b>

            <?php elseif ($model->item->type === Item::TYPE_VARIABLE_TEMPERATURE): ?>

                Значение температуры <b><?= $model->item->name ?></b> было <b><?= $model->value ?> °С</b> в комнате <b><?= $model->item->widget->room->name ?></b>

            <?php elseif ($model->item->type === Item::TYPE_VARIABLE_HUMIDITY): ?>

                Значение влажности <b><?= $model->item->name ?></b> было <b><?= $model->value ?> %</b> в комнате <b><?= $model->item->widget->room->name ?></b>

            <?php elseif ($model->item->type === Item::TYPE_VARIABLE_BOOLEAN): ?>

                Значение элемента <b><?= $model->item->name ?></b> было <b><?= Yii::$app->formatter->asBoolean($model->value) ?></b> в комнате <b><?= $model->item->widget->room->name ?></b>
            ...
            <?php elseif ($model->item->type === Item::TYPE_VARIABLE_BOOLEAN_DOOR): ?>
                ...
                <?php else: ?>
                    закрыта
                <?php endif ?></b> в комнате <b><?= $model->item->widget->room->name ?></b>

            <?php else: ?>

                Значение элемента <b><?= $model->item->name ?></b> было <b><?= $model->value ?></b> в комнате <b><?= $model->item->widget->room->name ?></b>
